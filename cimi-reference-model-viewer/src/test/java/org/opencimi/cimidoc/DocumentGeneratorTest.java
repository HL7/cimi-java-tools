    ...
    public void generateDocument() throws Exception {
        ...
        model.getClassDefinitions().values().forEach(bmmClass -> {
            if(StringUtils.isBlank(bmmClass.getDocumentation())) {
                System.out.println("BmmClass " + bmmClass.getName() + " needs documentation!");
                fixesNeeded.add(true);
            }
            bmmClass.getProperties().values().forEach(bmmProperty -> {
                if(StringUtils.isBlank(bmmProperty.getDocumentation())) {
                    System.out.println(bmmClass.getName() + "." + bmmProperty.getName() + " needs documentation.");
                    fixesNeeded.add(true);
                }
            });
        });
        if(fixesNeeded.size() == 0) {
            generator.generateDocument(model);
        }
    }
